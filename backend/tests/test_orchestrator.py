...
external calls (clone, analyze, generate, build) without a job store,
...
def test_success_first_attempt(mock_analyze, mock_gen, mock_build):
    with _mock_clone():
...
@patch("app.agent.orchestrator.generate_dockerfile", return_value=_good_output())
@patch("app.agent.orchestrator.analyze_repo", return_value=MagicMock())
def test_success_returns_output_dict(mock_analyze, mock_gen, mock_build):
    events: list[dict] = []
    with _mock_clone():
...
@patch("app.agent.orchestrator.generate_dockerfile", return_value=_good_output())
@patch("app.agent.orchestrator.analyze_repo", return_value=MagicMock())
def test_success_emits_correct_step_order(mock_analyze, mock_gen, mock_build):
    with _mock_clone():
        events = _collect_events()

    types = _event_types(events)
    # clone → analyze → attempt → generate → build → build_result → done
    assert types.index("step_started") < types.index("done")
    step_events = [e for e in events if e["type"] == "step_started"]
    steps = [e["step"] for e in step_events]
    assert steps == ["clone", "analyze", "generate", "build"]


@patch("app.agent.orchestrator.build_image", return_value=_good_build())
@patch("app.agent.orchestrator.generate_dockerfile", return_value=_good_output())
@patch("app.agent.orchestrator.analyze_repo", return_value=MagicMock())
def test_success_first_attempt_number_is_one(mock_analyze, mock_gen, mock_build):
    with _mock_clone():
        events = _collect_events()
...
@patch("app.agent.orchestrator.analyze_repo", return_value=MagicMock())
def test_retry_succeeds_on_second_attempt(mock_analyze, mock_gen, mock_build, mock_fix):
...
@patch("app.agent.orchestrator.generate_dockerfile", return_value=_good_output("initial"))
@patch("app.agent.orchestrator.analyze_repo", return_value=MagicMock())
def test_retry_calls_fix_not_generate_on_attempt_2(mock_analyze, mock_gen, mock_build, mock_fix):
    mock_build.side_effect = [_bad_build(), _good_build()]

    with _mock_clone():
        _collect_events()

    # generate_dockerfile called once (attempt 1); fix_dockerfile called once (attempt 2)
...
@patch("app.agent.orchestrator.generate_dockerfile", return_value=_good_output("initial"))
@patch("app.agent.orchestrator.analyze_repo", return_value=MagicMock())
def test_retry_passes_error_tail_to_fix(mock_analyze, mock_gen, mock_build, mock_fix):
...
@patch("app.agent.orchestrator.generate_dockerfile", return_value=_good_output("initial"))
@patch("app.agent.orchestrator.analyze_repo", return_value=MagicMock())
def test_retry_emits_two_attempt_events(mock_analyze, mock_gen, mock_build, mock_fix):
    mock_build.side_effect = [_bad_build(), _good_build()]
...
def test_all_attempts_fail_error_has_attempt_history(mock_analyze, mock_gen, mock_build, mock_fix):
    ...
    assert len(error_event["attempt_history"]) == 3  # max_attempts = 3
...
def test_max_attempts_one_means_no_retry(mock_analyze, mock_gen, mock_build, mock_fix):
    ...
    assert mock_gen.call_count == 1
    assert mock_fix.call_count == 0  # no fix calls when max_attempts=1
